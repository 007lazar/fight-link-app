...
- **2026-05-06** — **Swagger + OpenAPI codegen adopted as the type strategy.** `@nestjs/swagger` added to backend. All DTOs decorated. Frontend generates TypeScript types via `npm run gen:types` (fetches `/docs-json` from the running backend, outputs `src/lib/api-types.ts`). This is the single source of truth for API types — hand-written frontend type files are legacy.
- **2026-05-06** — **`CreateEventDto` extended** with optional fields `address`, `capacity`, `registrationDeadline`, `gymId` — these existed in the Prisma schema but were missing from the DTO, so the frontend could not submit them.
- **2026-05-06** — **Create Gym and Create Event pages added to frontend.** Two-column layout with live preview on desktop, single-column on mobile. Badge chip input for gym tags. Discipline dropdown uses Phase 3 vocabulary values (future-proof). All form inputs are controlled so values survive failed submissions.
